...
#
# See LST_mkfit_Tutorial.md for detailed documentation
#
###############################################################################
...
opts.register('output', 'out_lst.root',
              vpo.VarParsing.multiplicity.singleton,
              vpo.VarParsing.varType.string,
              'path to output ROOT file (LST+mkfit tracking)')
...
###
print("="*80)
print("Applying LST + mkfit tracking customization (TRKv08)")
print("  - LST: GPU-accelerated pattern recognition")
print("  - mkfit: Parallelized track fitting (3.5-7x speedup)")
print("="*80)

from JMETriggerAnalysis.Common.hltPhase2_TRKv08_LST_mkfit import customise_hltPhase2_TRKv08_LST_mkfit

# Apply ONLY the tracking customization (without PF, JME, reconstruction dependencies)
# This avoids issues with missing offline reconstruction sequences
process = customise_hltPhase2_TRKv08_LST_mkfit(process)

print("LST + mkfit tracking customization applied successfully")
print("="*80)
...
#  + process.offlinePrimaryVerticesMultiplicity
  ...
  + process.hltLSTmkfitRecoSequence  # LST+mkfit reconstruction (PF, jets, MET)
)
...
#    hltPFPuppiNoLep = cms.InputTag('hltPFPuppiNoLep'),
    ...
    # NOTE: With LST+mkfit tracking customization, the standard HLT collections
    # (hltAK4PFJets, hltPFMET, etc.) already use LST+mkfit tracks.
    # No separate LST-specific collections needed.
  ),
...
#     offlineAK4PFPuppiJetsCorrected = cms.InputTag('offlineAK4PFPuppiJetsCorrected') # with rerunPUPPI option
    ...
    # NOTE: With LST+mkfit tracking, the standard HLT jet collections above
    # already use LST+mkfit tracks (hltAK4PFJets, hltAK4PFPuppiJets, etc.)
  ),
...
#   hltPFClusterMET = cms.InputTag('hltPFClusterMET'),
  ...
  recoPFMETCollections = cms.PSet(
    ...
    # NOTE: With LST+mkfit tracking, the standard HLT MET collections above
    # already use LST+mkfit tracks (hltPFMET, hltPFPuppiMET, etc.)
  ),
...
# Continue processing even if products are not found (data format compatibility)
process.options.TryToContinue = cms.untracked.vstring('ProductNotFound', 'FileReadError')
